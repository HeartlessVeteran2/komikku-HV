...
class ReaderPageThumbnailCache(private val context: Context) {
    ...
    private val cacheDirectory = File(context.cacheDir, PARAMETER_CACHE_DIRECTORY)

    // Lazy: this class is a singleton constructed at app startup, so opening the DiskLruCache
    // (disk I/O, can throw IOException) must not happen eagerly on the main thread.
     ...
     * after [isThumbnailCached] returns true or [putThumbnail] has succeeded. Built directly
     * from [cacheDirectory] so callers can compute the path without forcing [diskCache] open.
     */
    fun getThumbnailFile(mangaId: Long, chapterId: Long, pageIndex: Int): File {
        val key = keyFor(mangaId, chapterId, pageIndex)
        return File(cacheDirectory, "$key.0")
    }
